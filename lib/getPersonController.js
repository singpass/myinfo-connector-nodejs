...
const securityController = require('../lib/securityController.js');
const privateKeyController = require('../lib/privateKeyController.js');
const requestHandlerController = require('../lib/requestHandlerController.js');
 ...
 */
let getPersonData = function (accessToken, txnNo, privateKey) {

  return new Promise((resolve, reject) => {
    securityController.verifyJWS(CONFIG.MYINFO_SIGNATURE_CERT_PUBLIC_CERT, accessToken)
      .then(decodedToken => {
        ...
        return callPersonAPI(uinfin, accessToken, txnNo, privateKey);
      })
      .then(personRes => {
        if (personRes && personRes.msg) {
          ...
          if (CONFIG.ENVIRONMENT === constant.ENVIRONMENT.SANDBOX) {
            ...
            return securityController.decryptJWE(privateKey, msg);
          }
        } else {
      ...
      .then(decryptResponse => {
        if (CONFIG.ENVIRONMENT === constant.ENVIRONMENT.SANDBOX) {
          ...
          if (!decryptResponse) {
            ...
            return securityController.verifyJWS(CONFIG.MYINFO_SIGNATURE_CERT_PUBLIC_CERT, decryptResponse);
          }
        }
 ...
 */
let callTokenAPI = function (authCode, privateKey, state) {
  ...
  if (CONFIG.ENVIRONMENT === constant.ENVIRONMENT.SANDBOX) {
    ...
    authHeaders = securityController.generateAuthorizationHeader(
      CONFIG.TOKEN_URL,
      params,
      method,
      contentType,
  ...
  return requestHandlerController.getHttpsResponse(tokenDomain, tokenRequestPath, headers, method, params);
};
 ...
 */
let callPersonAPI = function (sub, accessToken, txnNo, privateKey) {
  ...
  return requestHandlerController.getHttpsResponse(domain, requestPath, headers, method, null);
};
 ...
 * 
 * @param {string} accessToken - The access token from token API
 * @param {string} txnNo - Transaction ID from requesting digital services for cross referencing.
 * @returns {Promise} - Returns the Person Data (Payload decrypted + Signature validated)
 */
exports.getPersonData = function (accessToken, txnNo) {
  return new Promise((resolve, reject) => {
    privateKeyController.decryptPrivateKey(CONFIG.CLIENT_SECURE_CERT, CONFIG.CLIENT_SECURE_CERT_PASSPHRASE)
      .then(result => {
        logger.debug('Client Private Key: ', CONFIG.CLIENT_SECURE_CERT);
        let privateKey = (result.key);
        return getPersonData(accessToken, txnNo, privateKey);
      })
 ...
 * 
 * This method calls the Token API and obtain an "access
 ...
 * @returns {Promise} - Returns the Access Token
 */
exports.getAccessToken = function (authCode, state) {
  return new Promise((resolve, reject) => {
    privateKeyController.decryptPrivateKey(CONFIG.CLIENT_SECURE_CERT, CONFIG.CLIENT_SECURE_CERT_PASSPHRASE)
      .then(result => {
        logger.debug('Client Private Key: ', CONFIG.CLIENT_SECURE_CERT);
        ...
        return callTokenAPI(authCode, privateKey, state);
      })
      .then(tokenResult => {
